...
void loop(void){
  ...
  motorLeft->setSpeed(TESTSPEED); // set motor speeds
  ...
  delay(1000);
    ...
    motorLeft->setSpeed(TESTSPEED); // Reset motor speeds change by allStop.
  motorRight->setSpeed(TESTSPEED);
  motorLeft->run(RELEASE); // Now do the same thing backwards
  motorRight->run(FORWARD);
  delay(1250);
