...
void setup(void){
  ...
  delay(3000);  // Two second delay to get the robot ready
}

void loop(){
  motorLeft->setSpeed(FORWARD_SPEED);
  motorRight->setSpeed(FORWARD_SPEED-5);  
 while (digitalRead(LEFT_BUMP_PIN) != 1 &&
 digitalRead(RIGHT_BUMP_PIN) == 1) {
 ...
 if (digitalRead(LEFT_BUMP_PIN) != 1 &&
 digitalRead(RIGHT_BUMP_PIN) != 1) {
   motorLeft->setSpeed(BACKWARD_SPEED/2);
   motorRight->setSpeed(BACKWARD_SPEED);  
   motorLeft->run(BACKWARD);
   motorRight->run(BACKWARD);
   delay(TURN_DURATION);
   motorLeft->setSpeed(FORWARD_SPEED);
   motorRight->setSpeed(FORWARD_SPEED/3.3);
   motorLeft->run(FORWARD);
   motorRight->run(FORWARD);
   delay(650);
 }
